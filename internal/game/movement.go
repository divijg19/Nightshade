...
func ResolveMovement(pos world.Position, action agent.Action, worldWidth int,
	worldHeight int) world.Position {
	...
	if tgt.X < 0 || tgt.Y < 0 || tgt.X >= worldWidth || tgt.Y >= worldHeight {
		return pos
	}
	return tgt
}
