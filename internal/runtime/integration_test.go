...
func TestTwoHumansObservationAndMove(t *testing.T) {
	...
	// Provide input for agent A to move east ('d') and an empty input for B
	// to satisfy the server's blocking input contract.
	a.RecvInput <- "d"
	b.RecvInput <- ""

	// Run one tick. Provide inputs for the second tick as well to satisfy
	// the blocking TickOnce contract.
	_ = rt.TickOnce()
	// second tick: provide empty inputs for both agents
	a.RecvInput <- ""
	b.RecvInput <- ""
	_ = rt.TickOnce()
...
func TestContagionTransfer(t *testing.T) {
	...
	// Provide empty inputs for both RemoteHuman agents so TickOnce (which
	// blocks waiting for inputs) can proceed.
	a.RecvInput <- ""
	b.RecvInput <- ""
	_ = rt.TickOnce()
